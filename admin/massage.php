...
if (isset($_GET['delete'])) {
  ...
  $commentstmt = $conn->query("DELETE FROM table_comment WHERE commentID = $comment_id");
  $commentstmt->execute();

  if ($commentstmt) {
    echo "<script>alert('Data has been deleted successfully');</script>";
    $_SESSION['success'] = "Data has been deleted successfully";
    header("refresh:1; url=massage.php");
  }
}
?>
<!DOCTYPE html>
  ...
  <div class="btn-grp">
    ...
    <div class="head-text">
      ...
      <ul>
        ...
        <a href="massage.php">
          <li class="content-com">FEEDBACK</li>
        </a>
        <a href="user.php">
    ...
    <div class="btn-add">
      <?php $stmt = $conn->query("SELECT COUNT(*) as countFD FROM table_comment 
                            GROUP BY commentID");
      $stmt->execute();
      $count = $stmt->rowCount();
      ?>
      <p>มีความคิดเห็นทั้งหมด <?= $count; ?> ข้อความ</p>
    </div>
  </div>
  <table class="table">
    <thead>
    ...
    <tbody>
      ...
      $stmt = $conn->query("SELECT * FROM table_comment 
                                JOIN table_user ON table_comment.userID = table_user.userID 
                                ORDER BY table_comment.commentID
      ...
      ?>
          <tr>
            <th class="text-center" scope="row"><?= $massage['commentID']; ?></th>
            <td class="text-center" style="max-width: 400px; word-wrap: break-word; "><?= $massage['comment']; ?></td>
            <td class="text-center"><?= $massage['date']; ?></td>
            <td class="text-center"><?= $massage['firstName']; ?></td>
            <td class="text-center"><?= $massage['lastName']; ?></td>
            <td class="text-center"><?= $massage['username']; ?></td>
            <td class="text-center"><?= $massage['email']; ?></td>
            <td class="text-center"><?= $massage['role']; ?></td>
            <td class="text-center">
              <a href="?delete=<?= $massage['commentID']; ?>" class="btn btn-danger" onclick="return confirm('Are you sure ?');">Delete</a>
            </td>
          </tr>
      <?php }
      } ?>
